...
func (f Filter) compile(expr *lang.Expression) (clause.Expression, error) {
    ...
    case *lang.Expression_BinaryExpr:
        ...
        case "&&", "and":
            if left, err := f.compile(binary.LeftHandArgument); err != nil {
                return nil, err
            } else {
                if right, err := f.compile(binary.RightHandArgument); err != nil {
                    return nil, err
                } else {
        ...
        case "==", ">=", "<=", ">", "<":
            filed := &Field{}
            if err := filed.Parse(binary.LeftHandArgument); err != nil {
                return nil, err
            }
            value := &Value{}
            if err := value.Parse(binary.RightHandArgument); err != nil {
                return nil, err
            }
            switch binary.Operator.Symbol {
            case "==":
